...
import { getConnectedBrowserProvider } from '@/lib/ethereum-provider';
...
export const useDexSwap = () => {
    ...
    const loadUserBalances = useCallback(async () => {
        ...
        try {
            ...
            if (account?.address) {
                const connected = await getConnectedBrowserProvider();
                if (connected) {
                    browserProvider = connected.provider;
    ...
    const calculateOutput = useCallback(async () => {
        ...
        try {
            ...
            if (account?.address) {
                const connected = await getConnectedBrowserProvider();
                if (connected && connected.chainId === 11155111) {
    ...
    }, [swapForm.fromAmount, swapForm.fromToken, swapForm.toToken, account?.address]);
