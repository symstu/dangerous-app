...
export default function BuyCryptoPage() {
  ...
  const [isDemoGateway, setIsDemoGateway] = useState(false);
  ...
  useEffect(() => {
    ...
    loadRazorpayScript().then(({ loaded, isDemoGateway: demoGateway }) => {
      if (cancelled) return;
      setScriptLoaded(loaded);
      setIsDemoGateway(demoGateway);
      if (!loaded) {
        toast({
          variant: "destructive",
          title: "Error",
  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      const razorpayKey =
        process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID?.trim() || "rzp_demo";

      const orderId =
        orderResponse.data.orderId ?? orderResponse.data.razorpayOrderId;
      const orderAmount = orderResponse.data.amount;
      const orderCurrency = orderResponse.data.currency;

      if (!orderId || orderAmount == null || !orderCurrency) {
              ...
              <div className="relative group h-full">
                ...
                <div className="relative bg-slate-950/80 backdrop-blur-xl rounded-[26px] p-6 lg:p-8 border border-white/10 shadow-2xl h-full flex flex-col">
                  ...
                  <div className="space-y-6 flex-grow">
                    ...
                    {scriptLoaded && isDemoGateway && (
                      <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center space-x-3">
                        <Info className="h-5 w-5 text-amber-400" />
                        <p className="text-amber-200 text-sm">Demo payment mode — no real charge will be made.</p>
                      </div>
                    )}
                  </div>
