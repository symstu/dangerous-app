...
export function walletInstallUrl(provider: WalletProviderId): string {
  const store = detectBrowserExtensionStore();
  const urls = WALLET_EXTENSION_URLS[provider];
  if (urls) {
    return urls[store] ?? urls.chrome;
  }
  return WALLET_EXTENSION_URLS.metamask[store] ?? WALLET_EXTENSION_URLS.metamask.chrome;
}

type BrowserExtensionStore = "chrome" | "firefox" | "edge";

const WALLET_EXTENSION_URLS: Record<
  Exclude<WalletProviderId, "injected" | "demo">,
  Record<BrowserExtensionStore, string>
> = {
  metamask: {
    chrome:
  ...
  rabby: {
    ...
    edge: "https://microsoftedge.microsoft.com/addons/detail/rabby-wallet/acmacodkjbdgmoleebolmdjonilkdbch",
    firefox:
      "https://chromewebstore.google.com/detail/rabby-wallet/acmacodkjbdgmoleebolmdjonilkdbch",
  },
  phantom: {
...
export function detectBrowserExtensionStore(): BrowserExtensionStore {
